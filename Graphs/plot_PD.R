...
disease = "PD"
ADlist = read.table("PD_list.txt", header = T, sep='\t', stringsAsFactors=F)
  ...
  for(idx in 1:length(gset)){  #one GSE can contains multiple GPLs
    ...
    if(gpl_id1 == gpl_id){
      ...
      for(j in 1:length(gene_list)){
        ...
        if(sum(is.na(exp[case_idx])) >= length(exp[case_idx])/2) next
        ctl_idx <- grep(ctl,types, ignore.case=TRUE)
        if(sum(is.na(exp[ctl_idx])) >= length(exp[ctl_idx])/2) next
        pval <- NULL
        tryCatch( {
