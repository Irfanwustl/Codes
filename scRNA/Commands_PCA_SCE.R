...
pca<-read.csv("Klaus_Kaestner_HPAP_Jonathan_2017_9/data/F800HT/QC/Tables/All_Features/HPAP013.PCA_allF.csv",h=T,row.names = 1)
row.names(pca)<-pca[,1]
...
rowData(sce)$feature_symbol<-row.names(counts2)
sce <- sc3(sce, ks = 5, biology = TRUE)
plotPCA(sce,colour_by = "sc3_5_clusters")
dev.off()
...
rowData(sce)$feature_symbol<-row.names(counts3)
sce <- sc3(sce, ks = 5, biology = TRUE)
